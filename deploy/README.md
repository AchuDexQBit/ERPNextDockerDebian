...
**MariaDB and Redis:** [`compose.ghcr.yml`](./compose.ghcr.yml) starts **MariaDB**, **Redis**, and **ERPNext** on one network (`db` / `redis` hostnames). **`RFP_DB_ROOT_PASSWORD`** in your env file must match the MariaDB root in that stack. ERPNext **`sites/`** is on a named volume (`frappe_sites`) so site files survive container recreation. The image **must not** create an empty **`sites/<RFP_DOMAIN_NAME>/`** folder before **`bench new-site`** (older entrypoints did); Frappe then reports **`Site … already exists`** even without **`site_config.json`**. Current **`setup.sh`** removes incomplete site dirs and retries **`new-site`** automatically; **`RFP_RECOVER_ORPHAN_SITE=true`** is only needed if MariaDB still holds a leftover database. For **external** DB/Redis only, edit the compose file (remove `db`/`redis` and `depends_on` as needed), mount **`frappe_sites`** (or equivalent) on ERPNext, and set **`RFP_DB_HOST`** / **`RFP_REDIS_URL`** to real endpoints.
